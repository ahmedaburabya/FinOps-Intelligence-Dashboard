...
class AggregatedCostDataBase(BaseModel):
    ...
    project: Optional[
        str
    ]  # Made optional to handle cases where project.id might be NULL
    sku: str
...
class LLMInsight(LLMInsightBase):
    ...
    id: int
    timestamp: (
        datetime  # Renamed from 'created_at' in the DB model for clarity in schema
    )
    created_at: datetime
    updated_at: datetime
...
class AIInsightRequest(BaseModel):
    ...
    insight_type: str = (
        "natural_query"  # e.g., "natural_query", "summary", "anomaly", "prediction", "recommendation"
    )
