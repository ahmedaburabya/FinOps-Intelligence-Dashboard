...
class LLMService:
    ...
    def _initialize(self):
        ...
        try:
            ...
            logger.info(f"Google Generative AI client initialized successfully with model: {LLM_MODEL_NAME}")
        except Exception as e:
            logger.error(f"Failed to initialize Google Generative AI client: {e}")
            raise RuntimeError(f"Critical: Failed to initialize Google Generative AI client. Error: {e}")
    ...
    async def _generate_with_gemini(self, prompt: str) -> str:
        ...
        try:
            ...
            if not response.candidates:
                raise ValueError("Google Generative AI returned no response candidates.")
            return response.text
        except Exception as e:
            logger.error(f"Error generating content with Google Generative AI: {e}")
            raise

    async def generate_spend_summary(self, aggregated_data: List[schemas.AggregatedCostData]) -> str:
        """
        Generates a natural language summary of cloud spend based on aggregated data using Google Generative AI.
        """
        prompt = self._format_data_for_llm(aggregated_data, "summarize the cloud spend trends and key cost drivers")
        return await self._generate_with_gemini(prompt)

    async def detect_anomalies(self, aggregated_data: List[schemas.AggregatedCostData]) -> str:
        """
        Analyzes aggregated data for unusual spending patterns or anomalies using Google Generative AI.
        """
        prompt = self._format_data_for_llm(aggregated_data, "identify any unusual spending patterns or anomalies and explain them")
        return await self._generate_with_gemini(prompt)

    async def generate_cost_optimization_recommendations(self, aggregated_data: List[schemas.AggregatedCostData]) -> str:
        """
        Provides recommendations for optimizing cloud costs based on aggregated data using Google Generative AI.
        """
        prompt = self._format_data_for_llm(aggregated_data, "provide specific and actionable cost optimization recommendations")
        return await self._generate_with_gemini(prompt)
    ...
    def _format_data_for_llm(self, aggregated_data: List[schemas.AggregatedCostData], context: str) -> str:
        ...
        if len(full_formatted_data) > MAX_LLM_INPUT_CHARS:
            ...
            if last_newline != -1:
                truncated_formatted_data = truncated_formatted_data[:last_newline] + "\n..."
            else:
                truncated_formatted_data += "..." # Just append ellipsis if no newline found
