...
def get_aggregated_cost_data_by_id(db: Session, cost_data_id: int):
    ...
    """
    return db.query(models.AggregatedCostData).filter(models.AggregatedCostData.id == cost_data_id).first()
...
def create_aggregated_cost_data(db: Session, cost_data: schemas.AggregatedCostDataCreate):
    """
...
def get_llm_insight_by_id(db: Session, insight_id: int):
    ...
    """
    return db.query(models.LLMInsight).filter(models.LLMInsight.id == insight_id).first()
...
def get_llm_insights(
    ...
    if related_finops_data_id:
        query = query.filter(models.LLMInsight.related_finops_data_id == related_finops_data_id)
    if start_date:
