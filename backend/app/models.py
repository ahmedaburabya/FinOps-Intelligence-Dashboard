...
class AggregatedCostData(Base):
    ...
    currency = Column(String, default="USD", nullable=False, comment="Currency of the aggregated cost")
    usage_amount = Column(Float, nullable=True, comment="Amount of usage for the SKU")
    usage_unit = Column(String, nullable=True, comment="Unit of usage (e.g., 'hour', 'GB')")
    created_at = Column(DateTime, default=datetime.utcnow, comment="Timestamp when this record was created")
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, comment="Timestamp when this record was last updated")
...
class LLMInsight(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    insight_type = Column(String, nullable=False, comment="Type of insight (e.g., 'spend_summary', 'anomaly_detection', 'cost_optimization')")
    insight_text = Column(String, nullable=False, comment="The actual text content of the insight generated by the LLM")
    ...
    related_finops_data_id = Column(Integer, ForeignKey("aggregated_cost_data.id"), nullable=True, comment="Foreign key to AggregatedCostData if the insight is related to specific cost data")
    
    timestamp = Column(DateTime, default=datetime.utcnow, nullable=False, comment="Timestamp when the insight was generated")
    sentiment = Column(String, nullable=True, comment="Optional sentiment of the insight (e.g., 'positive', 'negative', 'neutral')")
    created_at = Column(DateTime, default=datetime.utcnow, comment="Timestamp when this record was created")
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow, comment="Timestamp when this record was last updated")
    ...
    related_finops_data = relationship("AggregatedCostData", back_populates="llm_insights")
